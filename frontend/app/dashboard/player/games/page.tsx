...
function GameActivityCard({ game, hostMode }: { game: MatchmakingGame; hostMode: boolean }) {
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-4"><Link className="rounded-xl bg-sportGreen px-4 py-2.5 text-sm font-black text-white" href={isHost ? `/dashboard/player/games/${game.id}` : `/find-game/${game.id}`}>{isHost ? "Manage Game" : "View Game"}</Link>{game.user_state.room_access && game.user_state.room_access !== "NONE" ? <Link className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-black text-slate-700" href={`/dashboard/player/games/${game.id}/room`}>{game.game_type === "FILL_SQUAD" ? (game.is_booking_verified ? "Squad Room" : "Squad Planning") : (game.is_booking_verified ? "Open Game Room" : "Planning Room")}</Link> : null}</div>
    </article>
  );
}
