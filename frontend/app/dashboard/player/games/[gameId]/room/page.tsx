...
export default function GameRoomPage() {
  ...
  async function recordAttendance(participantId: number, attendanceStatus: "ATTENDED" | "ABSENT") {
    ...
    try {
      const response = await api.post<{ attendance: { id: number; status: ParticipationAttendanceStatus; review_deadline_at: string | null } }>(
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
          ...
          {game.status === "COMPLETED" && game.user_state.is_host ? <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black text-sportNavy">Attendance</h2><p className="mt-1 text-sm font-semibold text-slate-600">Record registered players after the game. A no-show report stays reviewable for 24 hours.</p><div className="mt-4 space-y-2">{game.participants.filter((participant) => participant.user !== null && participant.participant_type !== "GUEST").map((participant) => <AttendanceRow key={participant.id} action={attendanceAction} participant={participant} onAttendance={(status) => void recordAttendance(participant.id, status)} />)}</div></section> : null}
...
function formatAttendanceStatus(status: string) { return status.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase()); }
