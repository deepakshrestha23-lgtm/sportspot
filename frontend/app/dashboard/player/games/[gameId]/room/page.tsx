...
export default function GameRoomPage() {
  ...
  return (
    <div className="space-y-5">
      <DashboardPageHeader backHref={`/dashboard/player/games/${game.id}`} backLabel="Back to game" eyebrow={roomEyebrow(roomAccess, isFillSquad)} title={isFillSquad ? (isPlanning ? "Fill My Squad Planning" : "Fill My Squad Room") : (isPlanning ? "Pickup Planning Room" : "Pickup Game Room")} description={`${game.title} - ${game.booking_display_time}`} actions={game.user_state.is_host && !isReadOnly ? <Link className="sport-secondary-button" href={`/dashboard/player/games/${game.id}`}>Manage Game</Link> : undefined} />
