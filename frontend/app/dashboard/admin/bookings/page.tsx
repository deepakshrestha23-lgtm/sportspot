...
export default function AdminBookingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && !error && bookings.length ? <AdminPanel className="overflow-hidden"><div className="admin-table-scroll"><table className="admin-data-table admin-bookings-table"><colgroup><col className="admin-bookings-id" /><col className="admin-bookings-schedule" /><col className="admin-bookings-customer" /><col className="admin-bookings-payment" /><col className="admin-bookings-state" /><col className="admin-bookings-action" /></colgroup><thead><tr><th>Booking</th><th>Schedule</th><th>Customer</th><th>Payment</th><th>Booking state</th><th><span className="sr-only">Action</span></th></tr></thead><tbody>{bookings.map((booking) => <tr key={booking.id}><td data-label="Booking"><div className="min-w-0 text-left"><strong className="admin-primary-text">{booking.booking_code}</strong><span className="admin-secondary-text">{booking.venue.name} <span className="text-slate-300">·</span> {booking.court.name}</span></div></td><td data-label="Schedule"><div className="text-sm font-bold text-sportNavy"><span className="block">{formatCompactDate(booking.slot_date)}</span><span className="admin-secondary-text">{formatTime(booking.slot_start_time)} - {formatTime(booking.slot_end_time)}</span></div></td><td data-label="Customer"><div className="min-w-0 text-left"><strong className="admin-primary-text">{booking.player.name}</strong><span className="admin-secondary-text">{booking.player.email}</span></div></td><td data-label="Payment"><div className="flex flex-col items-end gap-1 sm:items-start"><strong className="text-base font-black text-sportNavy">NPR {Number(booking.amount).toLocaleString("en-NP")}</strong><AdminStatusPill value={booking.payment_status} tone={getAdminStatusTone(booking.payment_status)} />{booking.refund_status !== "NOT_REQUIRED" ? <span className="text-[11px] font-bold text-slate-500">{formatAdminValue(booking.refund_status)}</span> : null}</div></td><td data-label="Booking state"><div className="flex flex-col items-end gap-1 sm:items-start"><AdminStatusPill value={booking.status} tone={getAdminStatusTone(booking.status)} /><span className="text-[11px] font-bold text-slate-500">{booking.check_in ? "Checked in" : "Not checked in"}</span></div></td><td data-label="Action">{["RESERVED", "CONFIRMED"].includes(booking.status) ? <button className="admin-row-action admin-row-action-danger" onClick={() => setCancelTarget(booking)} type="button">Cancel</button> : <span className="admin-no-action">No action</span>}</td></tr>)}</tbody></table></div><AdminPaginationControls hasMore={pagination?.has_more || false} isLoading={isLoading} onNext={() => setPage((current) => current + 1)} onPrevious={() => setPage((current) => Math.max(current - 1, 1))} page={page} pageSize={pagination?.page_size || 25} total={pagination?.total || 0} /></AdminPanel> : null}
