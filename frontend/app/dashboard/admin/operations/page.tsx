...
export default function AdminOperationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && !error && items.length ? <section aria-labelledby="operation-list-heading"><div className="admin-operation-list-heading"><div><h2 id="operation-list-heading">{activeTab.label} activity</h2><p>Read-only monitoring. Changes stay in the source player, captain, venue, or scorer workflow.</p></div></div><div className="admin-operation-list">{items.map((item) => <OperationRow item={item} key={`${item.kind}-${item.id}`} />)}</div>{(pagination?.total || 0) > (pagination?.page_size || 25) ? <div className="admin-operation-pagination"><AdminPaginationControls hasMore={pagination?.has_more || false} isLoading={isLoading} onNext={() => setPage((current) => current + 1)} onPageChange={setPage} onPrevious={() => setPage((current) => Math.max(current - 1, 1))} page={page} pageSize={pagination?.page_size || 25} total={pagination?.total || 0} /></div> : null}</section> : null}
    </div>
  );
}

function OperationRow({ item }: { item: AdminOperationItem }) {
  return <article className="admin-operation-row"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="sport-eyebrow text-[10px]">{formatAdminValue(item.kind)}</span><AdminStatusPill tone={getAdminStatusTone(item.status)} value={item.status} /></div><h3 className="mt-2 break-words text-base font-bold text-sportNavy">{item.title}</h3><p className="mt-1 text-sm text-slate-500">{item.subtitle} · Managed by {item.owner}</p></div><p className="shrink-0 text-xs font-medium text-slate-400">Updated {formatDateTime(item.updated_at)}</p></div><dl className="admin-operation-meta"><Info label="Schedule" value={formatSchedule(item.schedule)} /><Info label="Location" value={item.location} /><Info label="Context" value={item.meta.join(" · ")} /></dl></article>;
}

function Info({ label, value }: { label: string; value: string }) { return <div className="min-w-0"><dt>{label}</dt><dd>{value}</dd></div>; }
