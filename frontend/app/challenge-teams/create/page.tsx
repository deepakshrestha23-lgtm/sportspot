...
function CreateTeamChallengeContent() {
  ...
  const selectedServiceArea: ServiceAreaSelection | null = areaCode && area && district
    ? { code: areaCode, area, district }
    : null;
  ...
  async function submit() {
    ...
    if (mode === "PLAN_FIRST" && (!date || !startTime || !endTime || !selectedServiceArea)) {
      setScheduleError("Complete the proposed date, start time, end time, and match area before sending.");
      setStep(2);
    ...
    try {
      const response = await api.post<{ challenge: { id: number } }>("/api/team-challenges/challenges/", {
        ...
        preferred_area: area,
        preferred_area_code: mode === "PLAN_FIRST" ? areaCode : "",
