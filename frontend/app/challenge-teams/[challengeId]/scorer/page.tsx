...
export default function CricketScorerPage() {
  ...
  const [wicket, setWicket] = useState({ kind: "BOWLED" as WicketKind, dismissedPlayerId: "", fielderId: "", incomingPlayerId: "" });
  ...
  async function saveWicket() {
    if (!room || !wicket.dismissedPlayerId) return;
    await mutate(`/api/scoring/fixtures/${room.fixture.id}/deliveries/${panel === "correct-wicket" ? "edit/" : ""}`, {
      ...
      incoming_batsman_id: wicket.incomingPlayerId ? Number(wicket.incomingPlayerId) : null,
    });
  }
...
function ScoringConsole({ innings, canOperate, isSaving, lastDelivery, panel, setPanel, onRun, onUndo }: { innings: Innings; canOperate: boolean; isSaving: boolean; lastDelivery: Delivery | null; panel: "extras" | "wicket" | "correct" | "correct-extra" | "correct-wicket" | null; setPanel: (value: "extras" | "wicket" | "correct" | "correct-extra" | "correct-wicket" | null) => void; onRun: (runs: number, correction: boolean) => void; onUndo: () => void }) {
