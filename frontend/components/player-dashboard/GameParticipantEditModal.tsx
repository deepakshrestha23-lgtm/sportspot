  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/50 p-0 sm:items-center sm:p-4" role="presentation">
      <section aria-labelledby="edit-participant-title" aria-modal="true" className="w-full max-w-md rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-6" role="dialog">
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button className="min-h-11 rounded-xl border border-slate-200 px-5 text-sm font-black text-slate-600" disabled={isSaving} onClick={onClose} type="button">Cancel</button><button className="min-h-11 rounded-xl bg-sportGreen px-5 text-sm font-black text-white disabled:opacity-60" disabled={isSaving} onClick={() => onSave({ role, ...(isGuest ? { guest_name: guestName.trim() } : {}) })} type="button">{isSaving ? "Saving..." : "Save participant"}</button></div>
      </section>
    </div>
  );
}
